...
def shrunk_url(url, shrunk_list=['http://','https://', "ftp://"]):
...
def domain_url(url):
    ...
    url = url.split(".")[-2:] #Then split by the . #might not work for UK co.uk
    url = ".".join(url) #
    return url
...
def url_list(url):
    ...
    input_ = url.split("//") #Do not reserved words
    if len(input_) > 2:
...
url = "ftp://one.example.com/help/me"
